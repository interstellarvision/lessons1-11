...
class WooFi:
    ...
    def swap_eth_to_usdc(self, amount: TokenAmount, slippage: float = 1):
        ...
        min_to_amount = TokenAmount(
            amount=eth_price * float(amount.Ether) * (1 - slippage),
            decimals=6
        )
    ...
    def swap_usdc_to_eth(self, amount: Optional[TokenAmount] = None, slippage: float = 1):
        if not amount:
            balance = self.client.balance_of(contract_address=WooFi.usdc_address)

            #print(f"🔍 Полученный баланс USDC: {balance} (type: {type(balance)})")  # Отладочный print

            if balance is None:
                raise ValueError("Не удалось получить баланс USDC. Проверь подключение к сети.")

              # Если баланс уже TokenAmount, не создаем новый объект
            if isinstance(balance, TokenAmount):
                amount = balance
            else:
                amount = TokenAmount(
                    amount=balance,
        ...
        amount=TokenAmount(amount=amount, decimals=self.client.get_decimals(contract_address=WooFi.usdc_address), wei=False)
        eth_price = self.client.get_eth_price(token='ETH')
        min_to_amount = TokenAmount(
